...
#define MAVLINK_STORM32_XML_HASH -5868706952538430796
...
typedef enum MAV_STORM32_GIMBAL_DEVICE_FLAGS
{
   ...
   MAV_STORM32_GIMBAL_DEVICE_FLAGS_RC_MIXED=2048, /* RC control. The RC input signal fed to the gimbal device is mixed into the gimbal's orientation. Is overridden by RC_EXCLUSIVE flag if that is also set. | */
...
/** @brief Flags for gimbal manager operation. Used for setting and reporting, unless specified otherwise. If a setting is accepted by the gimbal manager, is reported in the STORM32_GIMBAL_MANAGER_STATUS message. */
